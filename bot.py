...
# Функции обработки (Импортируем внутри, чтобы ускорить запуск)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("👋 Бот запущен! Выберите действие:", reply_markup=main_keyboard)
...
async def handle_object(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("⏳ Обрабатываю... (первый раз может быть долго)")
    try:
        ...
        if context.user_data.get('mode') == 'remove':
            await update.message.reply_document(document=io.BytesIO(res), filename="result.png")
            return ConversationHandler.END
...
def run_bot():
    try:
        ...
        conv = ConversationHandler(
            ...
            fallbacks=[CommandHandler("start", start), MessageHandler(filters.Regex("Отмена"), start)],
        )
        app.add_handler(CommandHandler("start", start))
        app.add_handler(conv)
        # Отключаем стоп-сигналы для работы в потоке
...
if __name__ == "__main__":
    ...
    # Запускаем бота с задержкой 10 секунд
    threading.Thread(target=lambda: (time.sleep(10), run_bot()), daemon=True).start()
    # Flask запускается ПЕРВЫМ и в основном потоке
